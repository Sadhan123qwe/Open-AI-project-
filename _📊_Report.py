...
if not expenses_df.empty:
    ...
    fig_category_bar = px.bar(category_monthly_data, x="month", y="amount", color="category", barmode="group", title="Monthly Expenses")
...
if not expenses_df.empty and not income_df.empty:
    ...
    fig_stacked_bar = px.bar(stacked_data, x="month", y="amount", color="Type", barmode="stacked Income Expenses")
...
with st.sidebar:
    ...
    with st.expander("💬 Chat with SynBot", expended=False)
